...
test('creates a group and switches without leaking the previous matches',async({page,backend})=>{
 await login(page);await page.goto('/groups');await page.getByLabel('Group name',{exact:true}).fill('New group');await page.getByLabel('Group description').fill('Friendly football');await page.getByRole('button',{name:'Create group',exact:true}).click();await expect(page.getByText('New group · Selected')).toBeVisible();await page.getByRole('button',{name:'View matches',exact:true}).last().click();await expect(page.getByText('No matches yet')).toBeVisible();expect(backend.memberships.at(-1)?.role).toBe('owner');
});
